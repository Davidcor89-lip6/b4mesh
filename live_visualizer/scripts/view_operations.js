...
var center_and_zoom_element = function(target, scale_modifier = 4) {
    ...
    if (element_bbox === undefined)
        return; // d3/rendering not initialized yet

    const svg_bbox = d3.select("svg").select("g").node().getBBox();
    const scale = scale_modifier === null
        ? 1
        : transform.k + scale_modifier;

    const translate_to = {
        x : ( -element_bbox.x - element_bbox.width / 2) * scale + (svg_bbox.width) / 2,
        y : ( -element_bbox.y - element_bbox.height / 2) * scale + (svg_bbox.height) / 2
    };
